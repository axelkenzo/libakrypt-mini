...
/*! \brief Очистка внутреннего состояния контекста функции hmac. */
 int ak_hmac_context_clean( ak_hmac );
/*! \brief Изменение внутреннего состояния контекста функции hmac. */
 int ak_hmac_context_update( ak_hmac , const ak_pointer , const size_t );
/*! \brief Получение окончательного результата. */
 int ak_hmac_context_finalize( ak_hmac , const ak_pointer , const size_t , ak_pointer );
...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Контекст выработки производных ключей. */
/*! \details Для генерации ключей используется метод, регламентируемый рекомендациями
    по стандартизации Р 1323565.1.022-2018 и основанный на последовательном применении алгоритма
    выработки имитоставки hmac.

    Реализуемый алгоритм является последовательным, поскольку каждый последующий ключ зависит
    от предыдущего. Поэтому сложность выработки ключа с номером \f$ n \f$
    составляет \f$ O(n) \f$ операций.                                                              */
/* ----------------------------------------------------------------------------------------------- */
 typedef struct nkey {
 /*! \brief Структура для хранения промежуточного ключа. */
  struct hmac hctx;
 /*! \brief Временный буффер. */
  ak_uint8 buffer[96];
 /*! \brief Количество элементов в буффере. */
  ak_uint32 length;
 /*! \brief Номер текущего производного ключа. */
  ak_int32 key_number;
} *ak_nkey;

/* ----------------------------------------------------------------------------------------------- */
/*! \brief Функция создает и инициализирует контекст выработки производных ключей. */
 int ak_nkey_context_create( ak_nkey , ak_uint8 * , const size_t , ak_uint8 *, const size_t ,
                                                        ak_uint8 * , size_t , ak_uint8 * , size_t );
/*! \brief Функция формирует и возвращает указатель на новый ключ. */
 ak_uint8 *ak_nkey_context_generate_next_key( ak_nkey );
/*! \brief Функция возвращает номер сформированного ключа. */
 ak_int32 ak_nkey_context_get_key_number( ak_nkey );

/* ----------------------------------------------------------------------------------------------- */
 typedef struct bckey {
  /*! \brief Раундовые ключи, представляющие собой 10 векторов по 128 бит каждый. */
   ak_uint64 direct[20];
  /*! \brief Обратные раундовые ключи, используемые только для расшифрования информации. */
   ak_uint64 reverse[20];

} *ak_bckey;

/*! \brief Функция создает и инициализирует контекст блочного шифра Кузнечик. */
 int ak_bckey_context_create( ak_bckey , ak_uint8 * );
